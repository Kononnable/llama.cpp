...
// minimal logging in the format of llama-server:
//
//   0.00.035.060 I llm_load_tensors: ggml ctx size =    0.27 MiB
//   0.00.035.064 D ggml_backend_metal_log_allocated_size: ...
//
// i.e. [M.s.ms.us] L message, with colors when stderr is a terminal
...
static int64_t log_t_us() {
    return std::chrono::duration_cast<std::chrono::microseconds>(
        std::chrono::system_clock::now().time_since_epoch()).count();
}

static bool log_use_colors() {
#ifdef _WIN32
...
static int log_verbosity(enum ggml_log_level level) {
    switch (level) {
...
static void log_add(enum ggml_log_level level, const char * fmt, ...) {
    if (log_verbosity(level) > g_log_verbosity) {
        return;
    }
    ...
    if (level != GGML_LOG_LEVEL_NONE) {
        ...
        const int64_t ts = log_t_us() - g_log_t_start;
...
// route ggml log messages (from ggml-rpc.cpp) through the logger above
static void log_callback(enum ggml_log_level level, const char * text, void * /*user_data*/) {
    log_add(level, "%s", text);
}

// log macros, in the style of ggml (e.g. "ggml_cuda_init: ...")
#define RPC_DBG(fmt, ...) log_add(GGML_LOG_LEVEL_DEBUG, "%s: " fmt, __func__, __VA_ARGS__)
...
static void print_usage(int /*argc*/, char ** argv, rpc_server_params params) {
    ...
    fprintf(stderr, "  -lv, --verbosity, --log-verbosity N verbosity threshold: 1 = error ... 5 = debug (default: %d)\n", params.verbosity);
    fprintf(stderr, "\n");
}
...
static bool rpc_server_params_parse(int argc, char ** argv, rpc_server_params & params) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "-H" || arg == "--host") {
            ...
            g_log_verbosity = params.verbosity;
...
int main(int argc, char * argv[]) {
    ...
    g_log_t_start = log_t_us();
    g_log_colors  = log_use_colors();
